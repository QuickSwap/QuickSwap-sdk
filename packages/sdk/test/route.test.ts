...
describe('Route', () => {
  ...
  const weth = WETH[ChainId.ETHEREUM]
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], ETHER[ChainId.ETHEREUM])
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(ETHER[ChainId.ETHEREUM])
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, ETHER[ChainId.ETHEREUM])
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(ETHER[ChainId.ETHEREUM])
  })
})
