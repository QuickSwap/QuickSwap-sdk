...
/**
 * Exact address per chain, version and role, so a value recorded against the
 * wrong chain, version or role fails here rather than reaching a consumer.
 */
const DEPLOYMENT_FIXTURES: ReadonlyArray<DeploymentFixture> = [
  ...
  {
    // X Layer's quoter address is also Polygon's wrapped-native address. The
    // literal is shared; the contracts behind it are separate deployments on
    // separate chains.
    label: 'X Layer v3',
    chainId: XLAYER_ID,
...
describe('The deployment lookup', () => {
  ...
  describe('when an entry is read role by role', () => {
